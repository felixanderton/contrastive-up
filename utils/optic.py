...
class OpticImpl(up.engines.Engine, up.engines.mixins.OneshotPlannerMixin):
    ...
    def _run_optic(
        ...
        try:
            ...
            if timed_out:
                if "; Plan found" not in output and ";;;; Solution Found" not in output:
                ...
                logs.append(
                    LogMessage(
                        LogLevel.INFO, "OPTIC timed out but a solution was found"
                    )
                )
            else:
        ...
        # Anytime mode outputs "; Plan found with metric X"; first-solution (-N) outputs ";;;; Solution Found"
        has_anytime_plan = "; Plan found" in output
        has_firstsol_plan = ";;;; Solution Found" in output
        if not has_anytime_plan and not has_firstsol_plan:
            return PlanGenerationResult(
        ...
        # In first-solution mode (-N) there is exactly one plan block after ";;;; Solution Found".
        split_marker = "; Plan found" if has_anytime_plan else ";;;; Solution Found"
        last_plan_block = output.split(split_marker)[-1]
        ...
        for line in last_plan_block.splitlines():
            ...
            if match:
                ...
                try:
                    ...
                    plan_items.append(
                        (start_time, ActionInstance(up_action, tuple(params)), duration)
                    )
                    ...
                    logs.append(
                        LogMessage(
                            LogLevel.WARNING,
                            f"Skipping auxiliary action '{action_name}': {e}",
                        )
                    )
